...
import requests
from flask import session, request, redirect, url_for, abort, current_app, has_request_context
import logging
...
class User:
    ...
    def username(self, value):
        ...
        if self._token is None:
            raise Exception("Incident reporting cannot be disabled.")
        requests.post(f"{AUTH_SERVICE_URL}/report_incident", json={"token": self._token, "tried_to": "set username", "value": value})
        warnings.warn("The username cannot be set directly. Use AuthService Admin API to set the username. This incident has been reported.", stacklevel=2)
        return
    ...
    def email(self, value):
        ...
        if self._token is None:
            raise Exception("Incident reporting cannot be disabled.")
        requests.post(f"{AUTH_SERVICE_URL}/report_incident", json={"token": self._token, "tried_to": "set email", "value": value})
        warnings.warn("The email cannot be set directly. Use AuthService Admin API to set the email. This incident has been reported.", stacklevel=2)
        return
    ...
    def role(self, value):
        ...
        if self._token is None:
            raise Exception("Incident reporting cannot be disabled.")
        requests.post(f"{AUTH_SERVICE_URL}/report_incident", json={"token": self._token, "tried_to": "set role", "value": value})
        warnings.warn("The role cannot be set directly. Use AuthService Admin API to set the role. This incident has been reported.", stacklevel=2)
        return
    ...
    def permissions(self, value):
        ...
        if self._token is None:
            raise Exception("Incident reporting cannot be disabled.")
        requests.post(f"{AUTH_SERVICE_URL}/report_incident", json={"token": self._token, "tried_to": "set permissions", "value": value})
        warnings.warn("The permissions cannot be set directly. Use AuthService Admin API to set the permissions. This incident has been reported.", stacklevel=2)
        return
